...
import { Button, toast } from "@shopenup/ui"
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    ...
    await mutateAsync(payload, {
      onSuccess: async () => {
        ...
        // Check if any updated items are at or below threshold
        let hasLowStock = false
        for (const [inventory_item_id, item] of Object.entries(data.inventory_items)) {
          for (const [location_id, level] of Object.entries(item.locations)) {
            if (level.checked && level.quantity !== "") {
              const quantity = castNumber(level.quantity)
              const inventoryItem = items.find(i => i.id === inventory_item_id)
              const locationLevel = inventoryItem?.location_levels?.find(
                l => l.location_id === location_id
              )
        ...
        // Trigger low stock notification sync if needed
        if (hasLowStock) {
          try {
            ...
            console.warn('[Inventory Stock Update] Failed to sync low stock notifications:', error)
          }
        }
