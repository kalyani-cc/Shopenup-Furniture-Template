...
import { HttpTypes, InventoryLevelDTO, StockLocationDTO } from "@shopenup/types"
import { Button, Input, Text, toast } from "@shopenup/ui"
...
export const AdjustInventoryForm = ({
  ...
  const handleSubmit = form.handleSubmit(async (value) => {
    const newQuantity = castNumber(value.stocked_quantity)
    const availableQty = newQuantity - level.reserved_quantity
    ...
    await mutateAsync(
      ...
      },
      {
        onSuccess: async () => {
          ...
          // Check if quantity is at or below threshold and trigger low stock notification
          if (availableQty <= THRESHOLD) {
            try {
              // Call sync API to create notification if needed
              const response = await fetch(`${API_BASE_URL}/admin/custom/sync-low-stock-notifications`, {
                method: 'POST',
                credentials: 'include',
                headers: {
                ...
                body: JSON.stringify({ 
                  threshold: THRESHOLD,
                  force: false // Don't force - only create if notification doesn't exist
                }),
              })
              ...
              console.warn('[Inventory Update] Failed to sync low stock notifications:', error)
            }
          }
