...
const RepeatCustomerRateDetails = ({orderStatuses, dateRange, dateRangeCompareTo, compareEnabled} : 
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/admin/customers-analytics/repeat-customer-rate?${deduceDateUrlParams(dateRange, dateRangeCompareTo, orderStatuses).toString()}` , {
      credentials: "include",
    })
    .then((res) => res.json())
    .then((result) => {
  ...
  if (data && data.analytics.dateRangeFrom) {
    return (
      <Grid container style={{ width: '100%', maxWidth: '100%', margin: 0 }}>
        <Grid item xs={12} md={12} style={{ width: '100%', maxWidth: '100%', minWidth: 0, overflow: 'hidden' }}>
...
export const CustomersRepeatCustomerRate = ({orderStatuses, dateRange, dateRangeCompareTo, compareEnabled} : 
  {orderStatuses: OrderStatus[], dateRange?: DateRange, dateRangeCompareTo?: DateRange, compareEnabled: boolean}) => {
  return (
    <div style={{ width: '100%', maxWidth: '100%', minWidth: 0, boxSizing: 'border-box', paddingBottom: 16 }}>
      <Grid container spacing={3} style={{ width: '100%', maxWidth: '100%', margin: 0 }}>
        <Grid item xs={12} md={12} style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
          <Grid container spacing={2} style={{ width: '100%', maxWidth: '100%', margin: 0 }}>
