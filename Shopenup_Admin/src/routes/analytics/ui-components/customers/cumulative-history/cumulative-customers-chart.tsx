...
export const CumulativeCustomersChart = ({dateRange, dateRangeCompareTo, compareEnabled} : {dateRange?: DateRange, dateRangeCompareTo?: DateRange, compareEnabled?: boolean}) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/admin/customers-analytics/cumulative-history?${deduceDateUrlParams(dateRange, dateRangeCompareTo).toString()}` , {
      credentials: "include",
    })
    .then((res) => res.json())
    .then((result) => {
  ...
  if (data.analytics.dateRangeFrom && data.analytics.dateRangeTo) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 64, width: '100%', maxWidth: '100%', minWidth: 0 }}>
