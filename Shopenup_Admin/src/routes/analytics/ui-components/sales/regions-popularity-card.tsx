...
const RegionsPopularityDetails = ({orderStatuses, dateRange, dateRangeCompareTo, compareEnabled} : 
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/admin/sales-analytics/regions-popularity?${deduceDateUrlParams(dateRange, dateRangeCompareTo, orderStatuses).toString()}` , {
      credentials: "include",
    })
    .then((res) => res.json())
    .then((result) => {
  ...
  if (isLoading) {
    return (
      <TableContainer component={Paper} elevation={0} sx={{ background: 'transparent' }}>
        <Table size="small" aria-label="loading regions popularity">
          <TableBody>
    ...
    <div style={{ display: 'grid', gap: 12, paddingBottom: 8 }}>
      <div style={{ display: 'grid' }}>
        <Stack direction="row" spacing={1.5} alignItems={'center'}>
          <ShoppingBag/>
          <Heading level="h2">Regions popularity</Heading>
        </Stack>
      </div>
      <div style={{ display: 'grid' }}>
