...
const ProductsSoldCount = ({orderStatuses, dateRange, dateRangeCompareTo, compareEnabled} : {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/admin/products-analytics/sold-count?${deduceDateUrlParams(dateRange, dateRangeCompareTo, orderStatuses).toString()}` , {
      credentials: "include",
    })
    .then((res) => res.json())
    .then((result) => {
    ...
    <div style={{ display: 'grid', gap: 12, paddingBottom: 8 }}>
      <div style={{ display: 'grid' }}>
        <Stack direction="row" spacing={1.5} alignItems={'center'}>
          <ShoppingBag/>
          <Heading level="h2">Products sold</Heading>
        </Stack>
      </div>
      <div style={{ display: 'grid' }}>
