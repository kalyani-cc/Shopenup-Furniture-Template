...
import { Button, ProgressStatus, ProgressTabs, toast } from "@shopenup/ui"
...
import { compressImage } from "../../../../../hooks/api/use-image-upload"
...
export const ProductCreateForm = ({
  ...
  const handleSubmit = form.handleSubmit(async (values, e) => {
    ...
    // Normalize image URLs to ensure they have the correct format
    const normalizeImageUrl = (url: string | undefined): string => {
      if (!url) return ""
      
      // If URL already includes /static/, return as is
      if (url.includes('/static/')) {
        return url
      }
      
      // If URL is a full URL (starts with http:// or https://), check if it needs /static/
      if (url.startsWith('http://') || url.startsWith('https://')) {
        // If it's a full URL but doesn't have /static/, try to add it
        const urlObj = new URL(url)
        if (!urlObj.pathname.includes('/static/')) {
          const filename = url.split('/').pop()
          if (filename) {
            return `${urlObj.origin}/static/${filename}`
          }
        }
        return url
      }
      
      // If URL is relative or just a filename, construct full URL with /static/
      const filename = url.split('/').pop() || url
      const API_BASE_URL = import.meta.env.VITE_SHOPENUP_BACKEND_URL || ''
      
      // If we have a backend URL, use it; otherwise use current origin
      if (API_BASE_URL) {
        return `${API_BASE_URL}/static/${filename}`
      }
      
      return `${window.location.origin}/static/${filename}`
    }
    ...
    try {
      if (media.length) {
        ...
        const fileReqs = []
        if (thumbnailReq) {
          // Compress thumbnail image before uploading
          // Image will keep its original format (JPG stays JPG, PNG stays PNG)
          const compressedThumbnail = await compressImage(thumbnailReq.file, {
            maxWidth: 1920,
            maxHeight: 1920,
            quality: 0.92, // High quality (92%) for excellent visual quality
            // mimeType not specified - will preserve original format
          }).catch(() => thumbnailReq.file) // Fallback to original if compression fails

          fileReqs.push(
            sdk.admin.upload
              .create({ files: [compressedThumbnail] })
        ...
        if (otherMediaReq?.length) {
          // Compress all other images before uploading
          // Images will keep their original format (JPG stays JPG, PNG stays PNG)
          const compressedOtherMedia = await Promise.all(
            otherMediaReq.map(async (m) => {
              try {
                return await compressImage(m.file, {
                  maxWidth: 1920,
                  maxHeight: 1920,
                  quality: 0.92, // High quality (92%) for excellent visual quality
                  // mimeType not specified - will preserve original format
