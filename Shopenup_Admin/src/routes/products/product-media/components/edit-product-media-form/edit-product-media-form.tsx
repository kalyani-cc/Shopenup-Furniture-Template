...
import { ThumbnailBadge } from "@shopenup/icons"
import { HttpTypes } from "@shopenup/types"
import { Button, Checkbox, clx, CommandBar, toast, Tooltip } from "@shopenup/ui"
...
export const EditProductMediaForm = ({ product }: ProductMediaViewProps) => {
  ...
  const handleSubmit = form.handleSubmit(async ({ media }) => {
    ...
    if (filesToUpload.length) {
      // Compress all images before uploading to reduce file size while maintaining quality
    ...
    const withUpdatedUrls = media.map((entry, i) => {
      ...
      if (toUploadIndex > -1) {
        const uploadedUrl = uploaded[toUploadIndex]?.url
        const normalizedUrl = normalizeImageUrl(uploadedUrl)
        return { ...entry, url: normalizedUrl }
      }
      // Also normalize existing URLs that might be broken
      return { ...entry, url: normalizeImageUrl(entry.url) }
    })
