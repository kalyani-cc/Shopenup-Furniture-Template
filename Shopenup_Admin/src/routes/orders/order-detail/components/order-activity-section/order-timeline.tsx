import { Button, Text, Tooltip, clx, usePrompt } from "@shopenup/ui"
...
import { AdminOrderLineItem } from "@shopenup/types"
import { ShiprocketFulfillmentData } from "../../../../../hooks/api/fulfillment"
...
const useActivityItems = (order: AdminOrder): Activity[] => {
  ...
  const missingLineItemIds = getMissingLineItemIds(order, rmaChanges as AdminOrderChange[])
  const orderLineItemsQuery = useOrderLineItems(
    order.id,
    {
      fields: "+quantity",
    ...
    {
      enabled: !!rmaChanges.length && missingLineItemIds.length > 0,
    }
  )
  const removedLineItems = orderLineItemsQuery.order_items || []

  const itemsMap = useMemo(() => {
    const _itemsMap = new Map<string, AdminOrderLineItem>(order?.items?.map((i) => [i.id, i]))

    for (const id of missingLineItemIds) {
      const itemAny = removedLineItems.find((i: any) => i.item?.id === id) as any

      if (itemAny?.item) {
        // Cast to any to add quantity property
        const itemWithQuantity = { ...itemAny.item, quantity: itemAny.quantity || 0 } as AdminOrderLineItem
        _itemsMap.set(id, itemWithQuantity)
      }
    }
  ...
  const claims = (claimsQuery as any).claims || []

  const exchangesQuery = useExchanges({
    order_id: order.id,
    fields: "*additional_items",
  })
  const exchanges = (exchangesQuery as any).exchanges || []
  ...
  return useMemo(() => {
    ...
    for (const fulfillment of order.fulfillments || []) {
      const shiprocketData = (fulfillment.data as ShiprocketFulfillmentData) || {}
      const isShiprocket = !!shiprocketData.awb_code
      ...
      // Shiprocket-specific timeline events
      if (isShiprocket) {
        // Packed event
        if (shiprocketData.packed && shiprocketData.packed_at) {
          items.push({
            title: t("orders.activity.events.fulfillment.packed", "Marked as Packed"),
            timestamp: shiprocketData.packed_at,
            children: <ShiprocketActivityBody fulfillment={fulfillment} activityType="packed" />,
          })
        }

        // Pickup Requested event
        if (shiprocketData.pickup_requested_at) {
          items.push({
            title: t("orders.activity.events.fulfillment.pickupRequested", "Pickup Requested"),
            timestamp: shiprocketData.pickup_requested_at,
            children: <ShiprocketActivityBody fulfillment={fulfillment} activityType="pickup_requested" />,
          })
        }

        // Manifest Generated event
        if (shiprocketData.manifest_generated_at) {
          items.push({
            title: t("orders.activity.events.fulfillment.manifestGenerated", "Manifest Generated"),
            timestamp: shiprocketData.manifest_generated_at,
            children: <ShiprocketActivityBody fulfillment={fulfillment} activityType="manifest_generated" />,
          })
        }
      }
    ...
    for (const edit of orderChanges.filter((oc) => oc.change_type === "edit")) {
      ...
      items.push({
        ...
        timestamp: editTimestamp || edit.created_at,
    ...
    for (const update of orderChanges.filter(
      ...
      if (updateType === "shipping_address") {
        const oldAddress = update.actions[0]?.details?.old as any
        const newAddress = update.actions[0]?.details?.new as any
        
        items.push({
          title: (
            <ChangeDetailsTooltip
              title={t(`orders.activity.events.update_order.shipping_address`)}
              previous={oldAddress ? getFormattedAddress({ address: oldAddress }).join(", ") : "-"}
              next={newAddress ? getFormattedAddress({ address: newAddress }).join(", ") : "-"}
            />
          ),
          timestamp: update.created_at,
          children: updateAny.created_by ? (
            <div className="text-ui-fg-subtle mt-2 flex gap-x-2 text-sm">
              {t("fields.by")} <By id={updateAny.created_by} />
            </div>
          ) : null,
        })
      }
      ...
      if (updateType === "email") {
        const oldEmail = update.actions[0]?.details?.old as string
        const newEmail = update.actions[0]?.details?.new as string
    ...
    if (orderAny.canceled_at) {
      items.push({
        title: t("orders.activity.events.canceled.title"),
        timestamp: orderAny.canceled_at,
      })
    }
      ...
      >
        <div className="flex items-center justify-between">
          {itemsToSend?.length || itemsToReturn?.length ? (
            <ActivityItems
              title={typeof title === "string" ? title : String(timestamp)}
...
const FulfillmentCreatedBody = ({
  ...
  // Cast to any to access items which may exist on extended types
  const fulfillmentAny = fulfillment as any
  const items = fulfillmentAny.items || []
  
  const numberOfItems = items.reduce((acc: number, item: any) => {
  ...
  return (
    <div>
      ...
      {awbCode && (
        <Text size="small" className="text-ui-fg-subtle">
          AWB: {awbCode} {courierName ? `(${courierName})` : ""}
        </Text>
      )}
    </div>
  )
...
// Shiprocket Activity Body component
const ShiprocketActivityBody = ({
  fulfillment,
  activityType,
}: {
  fulfillment: AdminOrderFulfillment
  activityType: "packed" | "pickup_requested" | "manifest_generated"
}) => {
  const shiprocketData = getShiprocketData(fulfillment)
  const awbCode = shiprocketData.awb_code
  const courierName = shiprocketData.courier_name
  ...
  return (
    <div>
      ...
      {activityType === "pickup_requested" && shiprocketData.pickup_scheduled_date && (
        <Text size="small" className="text-ui-fg-subtle">
          Pickup Scheduled: {shiprocketData.pickup_scheduled_date}
        </Text>
      )}
      {activityType === "manifest_generated" && shiprocketData.manifest_id && (
        <Text size="small" className="text-ui-fg-subtle">
          Manifest ID: {shiprocketData.manifest_id}
        </Text>
      )}
    </div>
  )
...
const TransferOrderRequestBody = ({
  ...
  const originalEmail = (action.details as any)?.original_email as string | undefined

  return (
    <div>
      <Text size="small" className="text-ui-fg-subtle">
        {t("orders.activity.from")}: {originalEmail || "-"}
      </Text>
