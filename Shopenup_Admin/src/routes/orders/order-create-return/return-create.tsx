...
export const ReturnCreate = () => {
  ...
  useEffect(() => {
    async function run() {
      ...
      try {
        // Extract location_id from order fulfillments (same as backend/storefront)
        let locationId: string | undefined = undefined
        if (order.fulfillments && order.fulfillments.length > 0) {
          const fulfillment = order.fulfillments[0] as any
          locationId = fulfillment.location_id || undefined
        }

        // Initiate return with location_id if available
        const initiatePayload: any = { order_id: order.id }
        if (locationId) {
          initiatePayload.location_id = locationId
        }

        const orderReturn = await initiateReturn(initiatePayload)
