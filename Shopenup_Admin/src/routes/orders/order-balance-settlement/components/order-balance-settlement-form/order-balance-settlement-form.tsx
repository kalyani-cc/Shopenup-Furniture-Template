...
import { AdminOrder, AdminPayment } from "@shopenup/types"
import {
  Button,
  clx,
  CurrencyInput,
...
export const OrderBalanceSettlementForm = ({
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    ...
    if (data.settlement_type === "refund") {
      // Build payload - only include amount if it's not null (for partial refunds)
      const refundPayload: { amount?: number; note?: string } = {};
      
      if (data.refund?.amount.float !== null && data.refund?.amount.float !== undefined) {
        refundPayload.amount = data.refund.amount.float;
      }
      
      if (data.refund?.note) {
        refundPayload.note = data.refund.note;
      }
      ...
      await createRefund(
        ...
        {
          onSuccess: () => {
            const refundAmount = data.refund?.amount.float ?? 0;
            toast.success(
