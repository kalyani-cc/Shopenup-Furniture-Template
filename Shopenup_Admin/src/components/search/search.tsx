...
const CommandDialog = ({ children, ...props }: CommandDialogProps) => {
  ...
  return (
    <RadixDialog.Root {...props}>
      <RadixDialog.Portal>
        <RadixDialog.Overlay className="bg-ui-bg-overlay fixed inset-0 z-[9998]" />
        <RadixDialog.Content
          className={clx(
            "bg-ui-bg-base shadow-elevation-modal fixed left-[50%] top-[50%] z-[9999] flex max-h-[calc(100%-16px)] w-[calc(100%-16px)] min-w-0 max-w-2xl translate-x-[-50%] translate-y-[-50%] flex-col overflow-hidden rounded-xl p-0",
            {
        ...
        <div className="px-4 pt-4">
          <DropdownMenu>
            ...
            <DropdownMenu.Content
              align="start"
              className="h-full max-h-[360px] overflow-auto z-[10000]"
              onCloseAutoFocus={(e) => {
                e.preventDefault()
